      ...
      onRead_(file.onRead), onSeek_(file.onSeek), onClose_(file.onClose)
{
    ...
    try {
        ...
        const std::array<unsigned int, 5> hapVideoTags{
            MKTAG('H', 'a', 'p', '1'),
            MKTAG('H', 'a', 'p', '5'),
            MKTAG('H', 'a', 'p', 'Y'),
            MKTAG('H', 'a', 'p', 'M'),
        ...
        for (size_t i = 0; i < formatContext_->nb_streams; i++) {
            ...
            case AVMEDIA_TYPE_VIDEO:
                if (codecpar->codec_tag == desiredVideoTag
                    || std::find(hapVideoTags.begin(), hapVideoTags.end(), codecpar->codec_tag) != hapVideoTags.end()
                    || codecpar->codec_id == AV_CODEC_ID_HAP) {
                    videoStreamIdx_ = (int)i;

                    AVRational streamFrameRate = stream->r_frame_rate.num > 0 && stream->r_frame_rate.den > 0
                        ? stream->r_frame_rate
                        : stream->avg_frame_rate;
                    if (streamFrameRate.num <= 0 || streamFrameRate.den <= 0)
                        streamFrameRate = AVRational{ 30, 1 };

                    int64_t frameRateGCD = std::gcd(streamFrameRate.num, streamFrameRate.den);
                    auto encoderName = av_dict_get(stream->metadata, "encoder", nullptr, 0);

                    int64_t maxFrames = stream->nb_frames;
                    if (maxFrames <= 0 && stream->duration > 0)
                    {
                        maxFrames = av_rescale_q(
                            stream->duration,
                            stream->time_base,
                            AVRational{ streamFrameRate.den, streamFrameRate.num });
                    }
                    ...
                    video_ = VideoDef{
                        ...
                        codecpar->height,
                        VideoFormat{
                            static_cast<char>(codecpar->codec_tag & 0xFF),
                            static_cast<char>((codecpar->codec_tag >> 8) & 0xFF),
                            static_cast<char>((codecpar->codec_tag >> 16) & 0xFF),
                            static_cast<char>((codecpar->codec_tag >> 24) & 0xFF)
                        },
                        encoderName ? encoderName->value : "<unknown>",
                        codecpar->bits_per_coded_sample,
                        Rational{
                            (int)(streamFrameRate.num / frameRateGCD),
                            (int)(streamFrameRate.den / frameRateGCD)
                        },
                        maxFrames
                    };
                }
...
void MovieReader::readVideoFrame(int iFrame, std::vector<uint8_t>& frame)
{
    ...
    int64_t timestamp = (int64_t)iFrame * video_.frameRate.denominator * stream->time_base.den / (int64_t(video_.frameRate.numerator) * stream->time_base.num);
